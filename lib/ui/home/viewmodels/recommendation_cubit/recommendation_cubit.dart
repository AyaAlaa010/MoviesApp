...
class RecommendationCubit extends Cubit<RecommendationStates> {
  ...
  getRecommendationMovies() async {
    ...
    recommendationDataSource = RecommendationDataSourceImp(apiManager: apiManager);
    recommendationRepository = RecommendationRepositoryImp(recommendationDataSource);
    emit(LoadingRecommendationStates());
    try {
      List<MovieModel> RecommendationList = await recommendationRepository.getRecommendationMovies();
